...
def load_disaster_model(model_path: str = "disaster.h5"):
    ...
    # Resolve default model path relative to this file if needed
    ...
    # Import TensorFlow lazily to avoid crashing app startup if TF isn't installed yet
    try:
        from tensorflow.keras.models import load_model  # type: ignore
    except Exception as e:
        raise RuntimeError(
            f"TensorFlow is required to load the model but could not be imported: {e}")
...
def make_prediction(img_array: np.ndarray) -> Dict:
    ...
    try:
        ...
        predicted_class = CLASSES[top_idx] if top_idx < len(
            CLASSES) else f"class_{top_idx}"
